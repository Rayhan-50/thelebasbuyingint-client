  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        setError(e.response?.data?.message || 'Failed to load category. Please try again.');
        console.error('Error fetching category:', e.response?.data || e);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (error || !data) {
    return (
      <div className="text-center py-12">
        ...
        <Link
          to="/products"
          className="inline-block px-4 py-2 bg-deep-teal text-white rounded-lg hover:bg-[#70C5D7]"
        >
          Back to Products
        </Link>
      </div>
